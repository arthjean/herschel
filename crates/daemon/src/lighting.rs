...
//! [`nzxt_core::lighting::MIN_COMMAND_INTERVAL_MS`] after the previous one on
...
use nzxt_core::ipc::{ChannelState, HardwareState, LightingOutcome};
...
impl LightingExecutor {
    ...
    pub fn connected(
        ...
        channels: &[nzxt_core::capability::RgbChannel],
    ) -> Self {
        Self {
    ...
    /// Per-channel state for [`nzxt_core::ipc::DaemonStatus`].
...
mod tests {
    use super::*;
    use nzxt_core::capability::{Evidenced, RgbAccessory, RgbChannel};
    use nzxt_core::lighting::{Brightness, Rgb};
    use nzxt_hardware_linux::testing::FakeController;
    ...
    #[derive(Default)]
    struct Recorder {
        commands: std::sync::Mutex<Vec<[u8; nzxt_hardware_linux::rgb::REPORT_BYTES]>>,
    ...
    impl Recorder {
        ...
        fn last(&self) -> Option<[u8; nzxt_hardware_linux::rgb::REPORT_BYTES]> {
    ...
    impl HidTransport for RecordingController {
        fn write_report(
            &mut self,
            report: &[u8; nzxt_hardware_linux::rgb::REPORT_BYTES],
        ) -> Result<(), nzxt_hardware_linux::rgb::RgbError> {
            if self.recorder.fail.load(std::sync::atomic::Ordering::SeqCst) {
                return Err(nzxt_hardware_linux::rgb::RgbError::PermissionDenied {
                    path: "/dev/hidraw12".to_string(),
                });
            }
            self.inner.write_report(report)?;
            if [report[0], report[1]] == nzxt_hardware_linux::rgb::packet::COLOR_COMMAND
        ...
        fn read_report(
            ...
            Option<[u8; nzxt_hardware_linux::rgb::REPORT_BYTES]>,
            nzxt_hardware_linux::rgb::RgbError,
        > {
