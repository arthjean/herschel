...
use nzxt_core::capability::{Evidenced, RgbChannel, RgbTopology};
...
pub fn run<T: HidTransport + ?Sized, O: Operator>(
    ...
    Ok(WriteProbeReport {
        ...
        device: nzxt_core::RGB_CONTROLLER.to_string(),
...
mod tests {
    ...
    fn the_sweep_covers_every_value_the_screen_can_select() {
        ...
        for program in &programs {
            nzxt_core::lighting::validate_program(program)
    ...
    fn a_controller_that_refuses_every_write_is_reported_rather_than_retried() {
        let mut controller = FakeController::new("1.2.3", 1);
        controller.write_failure = Some(nzxt_hardware_linux::rgb::RgbError::PermissionDenied {
