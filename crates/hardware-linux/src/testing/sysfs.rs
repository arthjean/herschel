...
/// The configuration the development machine's devices are set to.
...
impl FakeSysfs {
    ...
    fn add_interface(&self, device: &Path, number: u8, class: u8, driver: Option<&str>) -> PathBuf {
        self.add_interface_in_configuration(device, DEFAULT_CONFIGURATION, number, class, driver)
    }

    /// An interface directory under an explicit `bConfigurationValue`.
    ///
    /// The configuration number is part of the directory name and is whichever
    /// one the kernel selected. It is a parameter here so a test can prove that
    /// nothing resolves an interface by guessing at that number.
    pub fn add_interface_in_configuration(
        &self,
        device: &Path,
        configuration: u8,
        number: u8,
        ...
        let interface = device.join(format!("{name}:{configuration}.{number}"));
    ...
    pub fn remove_kraken_hidraw(&self) {
        let interface = self
            .devices()
            .join(KRAKEN_BUS_ID)
            .join(format!("{KRAKEN_BUS_ID}:{DEFAULT_CONFIGURATION}.1"));
    ...
    pub fn remove_rgb_hidraw(&self) {
        let interface = self
            .devices()
            .join(RGB_BUS_ID)
            .join(format!("{RGB_BUS_ID}:{DEFAULT_CONFIGURATION}.0"));
